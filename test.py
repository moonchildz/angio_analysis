...
from utils.image_vis import draw_rpn_bbox_pred, draw_gt_boxes, draw_top_nms_proposals, draw_rpn_bbox_targets, \
    draw_rpn_bbox_pred_only
    ...
    for j in tqdm(range(0, num_batches_test)):
        ...
        if perform_watershed == 'yes':
            ...
            I8 = (((masks_watershed - masks_watershed.min()) / (
                        masks_watershed.max() - masks_watershed.min())) * 255).astype(np.uint8)
